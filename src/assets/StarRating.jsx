    ...
    <div style={container}>
      <div style={starContainer}>
        {Array.from({ length: maxRating }, (_, i) => (
          <Star
            key={i}
            onRateClick={() => rateHandler(i + 1)}
            full={hoverRating ? hoverRating >= i + 1 : rating >= i + 1}
            fullHover={() => setHoverRating(i + 1)}
            onRateHover={() => setHoverRating(i + 1)}
            onRateLeave={() => setHoverRating(0)}
            color={color}
            size={size}
          />
        ))}
        <p style={textStyle}>
          {messages.length === maxRating
            ...
            : `${hoverRating || rating || 0}/${maxRating}`}
        </p>
      </div>
    </div>
  );
...
function Star({ onRateClick, full, onRateHover, onRateLeave, size, color }) {
  const star = {
    width: size,
    height: size,
    display: "block",
